    ...
    <div className=" p-5">
      <div className="grid space-y-2 grid-cols-2 gap-2 w-full py-4">
        <div className="flex-1 -rotate-2 ">
          ...
          <p>{description} We are serious about your progress. If you commit yourself to never stop learnning we are with you any help you need. See in the course. Happy learning 🤞🏼</p>
          <p><h1 className="text-1xl font-bold">Syllabus:</h1>
        ...
        <div className="space-y-3 bg-primary-clr rotate-2 p-3 text-white">
          ...
          <div>
            ...
            {learningObjectives?.map((obj, idx) => (
              <p className="space-y-3" key={idx}>{obj}</p>
            ))}
          </div>
          {/* topics */}
          <div>
          ...
          <div className="space-y-1">
            {
                  topics?.map((topic, idx) => <p key={idx}>{topic}</p>)
            }
          </div>
          </div>
        </div>
       ...
       <div className="flex gap-2 pt-3 pb-8">
            {
                  reviews?.map((review,idx) => {
                        ...
                        return <div className="bg-[#4b4b4b1b] space-y-1 rounded shadow p-3" key={idx}>
                        <h1>{user}</h1>
                        <p>Ratings: {rating}</p>
                        <p>comment: {comment}</p>
                        </div>
      ...
      <img className="w-20 rounded-full h-20 object-cover" src="https://th.bing.com/th/id/R.edef308ba89f098b02e694b67411be6d?rik=%2fk3%2fC1v8RjKEvQ&pid=ImgRaw&r=0" alt="" />
      <div>
